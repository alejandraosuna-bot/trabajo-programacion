# Acta de Responsabilidad

## Proyecto: SimplePrestamos

### Integrantes

* María Alejandra Osuna Gil
* Mateo Guerra Londono

### Distribución de Responsabilidades

#### María Alejandra Osuna Gil

* creacion del repositorio.
* Elaboración de la documentación inicial.
* Desarrollo del módulo de usuarios.
* Realización de pruebas funcionales.
* Apoyo en la elaboración del informe final.

#### Mateo Guerra Londono

* Desarrollo del módulo de préstamos y devoluciones.
* Implementación de reportes.
* Corrección de errores.
* Apoyo en la documentación final.

### Cronograma General

| Actividad                  | Semana |
| -------------------------- | ------ |
| Planeación y documentación | 1      |
| Diseño del sistema         | 2      |
| Desarrollo de módulos      | 3 y 4  |
| Pruebas y correcciones     | 5      |
| Entrega final              | 5      |

### Criterios de Evaluación

* Cumplimiento de tareas asignadas.
* Participación activa en el proyecto.
* Calidad del trabajo realizado.
* Cumplimiento de los plazos establecidos.

### Fecha

27 / 5 / 2026

### Firma

---

María Alejandra Osuna Gil

---

Mateo Guerra Londono
